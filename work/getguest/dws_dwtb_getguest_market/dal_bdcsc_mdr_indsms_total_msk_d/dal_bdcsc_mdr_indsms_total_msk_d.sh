...
select t1.mdn,
       count(distinct(case
                        when t1.calling_sms_cnt > 0 then
                         t1.other_party
                      end)) calling_cnt,
       count(distinct(case
                        when t1.called_sms_cnt > 0 then
                         t1.other_party
                      end)) called_cnt,
       t1.day_id
  from (select temp1.mdn, other_party, calling_sms_cnt, called_sms_cnt, day_id
          from (select *
        ...
        select other_party     mdn,
               ...
               called_sms_cnt  calling_sms_cnt,
               calling_sms_cnt called_sms_cnt,
               day_id
          from (select *
                  from dal_bdcsc.dal_bdcsc_mdr_indsms_msk_d
                 where day_id = '${DAY_ID}') temp1
