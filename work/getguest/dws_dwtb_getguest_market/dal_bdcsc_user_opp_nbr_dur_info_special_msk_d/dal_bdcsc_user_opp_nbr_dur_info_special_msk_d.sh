...
###############配置区############################################################################
#脚本名称
LOGNAME="dal_bdcsc_user_opp_nbr_dur_info_special_msk_d"
...
#################################FOR循环开始##########################################################
###############################以下为SQL编辑区########################################################
RECENT_DAY_ID=$(hive -e "show partitions dws_integ.dws_dtfs_online_mbl_mix_info_d;"|awk -F '/' '{print $3}'|awk -F '=' '{print $2}'|sort -r|head -n 1)
#正常执行hql
SQL="add jar $baseDirForScriptSelf/md.jar;
CREATE TEMPORARY FUNCTION md5 AS 'com.udf.Md5';
insert overwrite table dal_bdcsc.dal_bdcsc_user_opp_nbr_dur_info_special_msk_d partition (month_id, day_id)
select t1.mdn,
       t1.other_party,
       t1.calling_dur,
       t1.calling_cnt,
  ...
  from (select mdn,
               other_party,
               sum(case
                     when call_type = '01' then
                      call_dur
                   end) calling_dur, --主叫时长
               count(case
                       when call_type = '01' then
                        mdn
                     end) calling_cnt, --主叫次数
               sum(case
                     when call_type = '02' then
                      call_dur
                   end) called_dur, --被叫时长
               count(case
                       when call_type = '02' then
               ...
               month_id,
               day_id
          from dws_m.dws_wdtb_cdr_mix_msk_d
         where month_id = '${MONTH_ID}'
           and day_id = '${DAY_ID}'
         group by mdn, other_party, month_id, day_id) t1
  left join (select *
               from dws_integ.dws_dtfs_online_mbl_mix_info_d
              where day_id = '${RECENT_DAY_ID}'
                and special_level in
                    ('1', '2', '3', '4', 'Y', 'SY', 'SN', '-1')) t2
    on t1.mdn = md5(t2.mdn)
  left join (select *
               from dws_integ.dws_dtfs_online_mbl_mix_info_d
              where day_id = '${RECENT_DAY_ID}'
                and special_level in
                    ('1', '2', '3', '4', 'Y', 'SY', 'SN', '-1')) t3
    on t1.other_party = md5(t3.mdn)
 where t2.mdn is null
   and t3.mdn is null;
"
