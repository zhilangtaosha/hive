...
###############配置区############################################################################
#脚本名称
LOGNAME="dal_bdcsc_user_opp_nbr_dur_info_total_msk_d"
...
#正常执行hql		 
SQL="insert overwrite table dal_bdcsc.dal_bdcsc_user_opp_nbr_dur_info_total_msk_d partition(day_id)
select t1.mdn,
       count(distinct(case
                        when t1.calling_cnt > 0 then
                         t1.other_party
                      end)) calling_cnt,
       count(distinct(case
                        when t1.called_cnt > 0 then
                         t1.other_party
                      end)) called_cnt,
       t1.day_id
  from (select temp1.mdn, other_party, calling_cnt, called_cnt, day_id
          from (select *
                  from dal_bdcsc.dal_bdcsc_user_opp_nbr_dur_info_special_msk_d
                 where day_id = '${DAY_ID}') temp1
          left join dwi_integ.dwi_evt_bill_other_num_list_info temp2
            on temp1.other_party = temp2.mdn_md5
         where temp2.mdn_md5 is null
        union all
        select other_party mdn,
               temp1.mdn   other_party,
               called_cnt  calling_cnt,
         ...
         where temp2.mdn_md5 is null
        union all
        select temp1.mdn, other_party, calling_cnt, called_cnt, day_id
          from (select *
                  from dal_bdcsc.dal_bdcsc_fix_opp_nbr_dur_info_special_msk_d
                 where day_id = '${DAY_ID}') temp1
          left join dwi_integ.dwi_evt_bill_other_num_list_info temp2
            on temp1.other_party = temp2.mdn_md5
         where temp2.mdn_md5 is null
        ...
        select other_party mdn,
          ...
          left join dwi_integ.dwi_evt_bill_other_num_list_info temp2
            on temp1.other_party = temp2.mdn_md5
         where temp2.mdn_md5 is null) t1
 group by t1.mdn, t1.day_id;
"
