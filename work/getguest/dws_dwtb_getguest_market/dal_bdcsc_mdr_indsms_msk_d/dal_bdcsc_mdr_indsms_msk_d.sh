...
###############配置区############################################################################
#脚本名称
LOGNAME="dal_bdcsc_mdr_indsms_msk_d"
...
insert overwrite table dal_bdcsc.dal_bdcsc_mdr_indsms_msk_d partition
  (day_id)
select t1.start_date,
       ...
       t1.called_sms_cnt,
       t1.day_id
  from (select substr(start_time, 1, 8) start_date, --业务发生时间
               mdn,
               other_party,
               count(case
                       when call_type = 'MO' then
                        mdn
                     end) calling_sms_cnt,
               count(case
                       when call_type = 'MT' then
                        mdn
                     end) called_sms_cnt,
               day_id
          from dws_m.dws_wdtb_mdr_mix_msk_d
         where day_id = '${DAY_ID}'
         group by mdn, other_party, day_id, substr(start_time, 1, 8)) t1
  left join (select *
               from dws_integ.dws_dtfs_online_mbl_mix_info_d
              where day_id = '${RECENT_DAY_ID}'
                and special_level in
                    ('1', '2', '3', '4', 'Y', 'SY', 'SN', '-1')) t2
    on t1.other_party = md5(t2.mdn)
  left join (select *
               from dws_integ.dws_dtfs_online_mbl_mix_info_d
              where day_id = '${RECENT_DAY_ID}'
                and special_level in
                    ('1', '2', '3', '4', 'Y', 'SY', 'SN', '-1')) t3
    on t1.mdn = md5(t3.mdn)
