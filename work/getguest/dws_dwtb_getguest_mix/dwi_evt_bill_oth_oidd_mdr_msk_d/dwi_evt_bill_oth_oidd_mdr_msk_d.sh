...
###############配置区############################################################################
#脚本名称
LOGNAME="dwi_evt_bill_oth_oidd_mdr_msk_d"
...
#正常执行hql
SQL="insert overwrite table dwi_m.dwi_evt_bill_oth_oidd_mdr_msk_d partition (prov_id,day_id)
select t1.mdn,
       t1.opposite_num other_party,
       t1.occur_time start_time,
       t1.call_type,
       t2.city_id calling_visit_code,
       '' called_visit_code, --对端号码记录“城市id”字段，没有则置空。
       t1.imsi,
       t1.bsid start_bsid,
       t1.lon start_lng,
       t1.lat start_lat,
  ...
  from (select mdn,
               opposite_num,
               occur_time,
               (case when call_sms_mm_type = '00' then 'MO'
               when call_sms_mm_type = '01' then 'MT' end) call_type,
               city_code,
               imsi,
               bsid,
               lon,
         ...
         where notice_id = '02'
           and temp1.prov_id in (select lpad(temp2.oidd_id,2,'0') oidd_id
                          from dim.dim_prov_match temp2
                         where temp2.prov_id = '${PROV_ID}') 
           and receive_day = '${DAY_ID}') t1
		   left join dim.dim_city t2
		   on concat('0',t1.city_code)=t2.area_code
 group by t1.prov_id,
          t1.receive_day,
          t1.mdn,
          t1.opposite_num,
