...
###############配置区############################################################################
#脚本名称
LOGNAME="dwi_evt_bill_oth_duty_mdr_msk_d"
...
insert overwrite table dwi_m.dwi_evt_bill_oth_duty_mdr_msk_d partition (prov_id, day_id)
select t1.biz_id,
       t1.CUST_NAME  Biz_cust_name,
       t3.city_id  Biz_area_code, --企业归属地,取自：客户资料信息中“区号”字段
       md5(t2.MO_NBR)     Mdn,
       t2.MT_NBR     other_party,
       t2.START_TIME start_date,
       t2.AREA_CODE  Long_area_code, --市长途区号
       t2.call_type,
       t2.PROV_ID,
       t2.day_id
  from (select biz_id,
               ...
               MT_NBR,
               START_TIME,
               if(substr(AREA_CODE, 1, 1) = 0,
                  AREA_CODE,
                  concat('0', AREA_CODE)) area_code,
               (case
                 when MSG_TYPE in ('00','10') then
                  'MO'
                 when MSG_TYPE in ('01','11') then
                  'MT'
               end) call_type,
               prov_id,
               day_id
          from indsmsbstl.industry_sms_billed
         where PROV_ID = '${PROV_ID}'
           and day_id = '${DAY_ID}') t2
  left join (select tt1.biz_id,
                    tt1.CUST_NAME,
                    if(substr(AREA_CODE, 1, 1) = 0,
                       AREA_CODE,
                       concat('0', AREA_CODE)) area_code,
                    tt1.PROV_ID,
                    tt1.day_id
               from indsmsbstl.industry_sms_cust_info tt1
              where PROV_ID = '${PROV_ID}'
                and days_id = '${DAY_ID}') t1
    on t1.biz_id = t2.biz_id
	left join dim.dim_city t3
