#!/bin/bash
#***********************************************************************************
# **  文件名称: dwi_res_netk_4g_station_list_m_mid.sh
# **  创建日期: 2018年6月14日
# **  编写人员: hgd
# **  输入信息: 
# **  输出信息: 
...
###############配置区############################################################################
#脚本名称
LOGNAME="dwi_res_netk_4g_station_list_m_mid"
...
#正常执行hql
SQL="
msck repair table bsabstl.bsa4g_station_d;

insert overwrite table dwi_i_mid.dwi_res_netk_4g_station_list_m_mid partition(month_id)
select t1.int_id, t1.longi, t1.lati, '${MONTH_ID}' month_id
  from (select int_id,
               longi,
               lati,
               row_number() over(partition by int_id order by day_id desc, longi desc, lati desc) rn
          from bsabstl.bsa4g_station_d
...
#合并小文件方法
Mergefile "${USERNAME}" "dwi_res_netk_4g_station_list_m_mid" "where month_id = '${MONTH_ID}'"
