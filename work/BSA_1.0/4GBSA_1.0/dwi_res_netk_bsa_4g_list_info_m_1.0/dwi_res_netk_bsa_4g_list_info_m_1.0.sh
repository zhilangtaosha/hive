#!/bin/bash
#***********************************************************************************
# **  文件名称: dwi_res_netk_bsa_4g_list_info_m.sh
# **  创建日期: 2018年6月14日
# **  编写人员: hgd
# **  输入信息: 
# **  输出信息: 
...
###############配置区############################################################################
#脚本名称
LOGNAME="dwi_res_netk_bsa_4g_list_info_m"
...
#正常执行hql
SQL="
insert overwrite table dwi_integ.dwi_res_netk_bsa_4g_list_info_m partition(month_id)
select t1.int_id,
       t1.cel_code,
       ...
       t1.month_id
  from (select *
          from dwi_i_mid.dwi_res_netk_cell_list_m_mid
         where month_id = '${MONTH_ID}') t1
  left join (select *
               from dwi_i_mid.dwi_res_netk_4g_station_list_m_mid
              where month_id = '${MONTH_ID}') t2
    on t1.related_enb_int_id = t2.int_id;"
RunScript "${SQL}"

#合并小文件方法
Mergefile "${USERNAME}" "dwi_res_netk_bsa_4g_list_info_m" "where month_id = '${MONTH_ID}'"
