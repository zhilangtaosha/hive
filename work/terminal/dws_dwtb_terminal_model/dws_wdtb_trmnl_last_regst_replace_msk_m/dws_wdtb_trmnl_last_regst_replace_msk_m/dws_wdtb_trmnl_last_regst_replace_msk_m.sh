...
###############配置区############################################################################
#脚本名称
LOGNAME="dws_wdtb_trmnl_last_regst_replace_msk_m.sh"
...
CREATE TEMPORARY FUNCTION MD5Encode AS 'com.bonc.hive.MyMD5';
INSERT OVERWRITE TABLE dws_m.dws_wdtb_trmnl_last_regst_replace_msk_m PARTITION (PROV_ID, MONTH_ID)
select md5encode(meid) as meid,
       ...
       exchange_cnt,
       trmnl_online_dur,
       trmnl_resident_avgdur,
       trmnl_first_resident_dur,
       trmnl_second_resident_dur,
       brand_continu_dur,
       brand_continu_avg_dur,
       prov_id,
       month_id
  from dws_integ.dws_wdtb_trmnl_last_regst_replace_m
