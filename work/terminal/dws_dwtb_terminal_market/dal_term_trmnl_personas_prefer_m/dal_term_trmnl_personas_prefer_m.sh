...
###############配置区############################################################################
#脚本名称
LOGNAME="dal_term_trmnl_personas_prefer_m"
...
#正常执行hql
SQL="insert overwrite table dal_term.dal_term_trmnl_personas_prefer_m partition (month_id)
select t1.month_id             month,
       t1.innet_trmnl_user_cnt,
       t1.innet_trmnl_cnt,
       t1.redion_name,
       t1.prov_name,
       t1.city_name,
  ...
  from (select tt1.prov_name,
               ...
               tt3.active_range,
               tt3.redion_name,
               tt2.brang_preference,
               tt2.type_preference,
               tt2.screen_size,
               -- tt1.meid,
               count(distinct (case when tt1.prod_inst_status in ('100000', '120000') then tt1.mdn end)) innet_trmnl_user_cnt,
         count(distinct (case when tt1.prod_inst_status in ('100000', '120000') then tt1.meid end)) innet_trmnl_cnt,
               tt1.month_id
          from (select mdn,
                       prov_name,
                       city_name,
                       ...
                       (case
                         when age > 0 and age <= 15 then
                          '(0,15]'
                         when age > 15 and age <= 25 then
                          '(15,25]'
          ...
          left join (select mdn,
                           ...
                           type_preference,
                           screen_size
                      from dwi_m.dwi_sev_user_last_regst_msk_m 
					  where month_id='${MONTH_ID}') tt2
            on tt1.mdn = tt2.mdn
          left join (select mdn,
                           residence_id,
                           workplace_id,
                           active_range,
                           redion_name
                      from dws_m.dws_wdtb_trmnl_place_msk_m
					  where month_id='${MONTH_ID}') tt3
            on tt1.mdn = tt3.mdn
         group by tt1.prov_name,
                  tt1.city_name,
                  tt1.trmnl_brand,
                  tt1.trmnl_model,
                  tt1.gender,
  ...
  left join (select dm_model,
                    ...
                    core_num,
                    dm_net_type
               from dim.dim_devinfo) t2
    on t1.trmnl_model = t2.dm_model;"
