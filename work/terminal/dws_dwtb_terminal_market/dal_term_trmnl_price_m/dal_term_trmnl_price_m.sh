#!/bin/bash
#***********************************************************************************
# **  文件名称: dal_term_trmnl_price_m.sh
# **  创建日期: 2018年04月09日
# **  编写人员: qinlin
# **  输入信息: 
# **  输出信息: 
...
###############配置区############################################################################
#脚本名称
LOGNAME="dal_term_trmnl_price_m.sh"
...
#正常执行hql
SQL=" 
INSERT OVERWRITE table dal_term.dal_term_trmnl_price_m PARTITION (MONTH_ID) 
select h.add_trmnl_cnt,
       h.add_trmnl_user_cnt,
       case
         when h.prov_name in ('广东', '广西', '福建', '海南') then
          '华南'
         when h.prov_name in ('江苏', '浙江', '上海', '台湾') then
          '华东'
         when h.prov_name in ('河南', '湖北', '安徽', '湖南', '江西') then
          '华中'
         when h.prov_name in ('西藏', '云南', '四川', '贵州', '重庆') then
          '西南'
         when h.prov_name in
              ('内蒙古', '山西', '河北', '北京', '天津', '山东') then
          '华北'
         when h.prov_name in ('新疆', '甘肃', '青海', '宁夏', '陕西') then
          '西北'
         when h.prov_name in ('黑龙江', '吉林', '辽宁') then
          '东北'
       END as redion_name,
       h.prov_name,
       h.city_name,
       h.trmnl_brand,
       h.trmnl_model trmnl_mdl,
       e.listed_price,
       e.model_point function_orientation,
       e.ios,
       e.standby_type,
       e.dm_net_frequency,
       h.trmnl_type,
       e.core_num,
       e.dm_net_type standard_type,
       h.add_trmnl_newuser_user add_trmnl_newuser_user,
       h.add_trmnl_newuser_user / h.add_trmnl_user_cnt trmnl_new_rate,
       h.add_newuser_user add_newuser_user,
       h.add_newuser_user / h.add_alluser_user user_new_rate,
       if(h.feeavg > 2000, '', h.feeavg) apru,
       case
         when h.prod_offer_fee > 0 and h.prod_offer_fee <= 50 then
          '(0,50]'
         when h.prod_offer_fee > 50 and h.prod_offer_fee <= 100 then
          '(50,100]'
         when h.prod_offer_fee > 100 and h.prod_offer_fee <= 150 then
          '(100,150]'
         when h.prod_offer_fee > 150 and h.prod_offer_fee <= 200 then
          '(150,200]'
         when h.prod_offer_fee > 200 and h.prod_offer_fee <= 300 then
          '((200,300]'
         when h.prod_offer_fee > 300 and h.prod_offer_fee <= 500 then
          '(300,500]'
         when h.prod_offer_fee > 500 and h.prod_offer_fee <= 1000 then
          '(500,1000]'
         when h.prod_offer_fee > 1000 then
          '(1000,∞)'
         else
          '0'
       END as offer_char_level,
       h.offeravg offer_char_avg,
       h.is_contract if_contract,
       h.month_id
  from (select a.prov_name,
               ...
               c.offeravg,
               count(distinct(case
                                when b.is_first_regst = 1 and substr(regexp_replace(b.regst_date,'-',''),1,6) = ${MONTH_ID} then
                                 a.meid
                              end)) add_trmnl_cnt, --新增终端数量
               count(distinct(case
                                when b.is_first_regst = 1 and substr(regexp_replace(b.regst_date,'-',''),1,6) = ${MONTH_ID} then
                                 a.mdn
                              end)) add_trmnl_user_cnt, --新增终端用户
               count(distinct(case
                                when substr(a.open_date,1,6) = ${MONTH_ID} then
                                 a.mdn
                              end)) add_newuser_user, --新开卡用户数
               count(distinct(case
                                when a.prod_inst_status in ('100000', '120000') then
                                 a.mdn
                              end)) add_alluser_user, --全量新开卡用户数
               count(distinct(case
                                when b.is_first_regst = 1 and substr(a.open_date,1,6) = ${MONTH_ID} then
                                 a.mdn
                              end)) add_trmnl_newuser_user --新增终端新开卡用户
          from (select mdn,
                       meid,
                       prov_name,
                       city_name,
          ...
          left join (select mdn, meid, regst_date, is_first_regst
                      from dws_m.dws_wdtb_trmnl_first_regst_stack_msk_m
                     where month_id = ${MONTH_ID}) b
            on a.mdn = b.mdn
           and a.meid = b.meid
          left join (select trmnl_brand,
                           trmnl_model,
                           avg(fee) feeavg,
                           avg(prod_offer_fee) offeravg
                      from dws_m.dws_wdtb_user_overall_info_msk_m
                     where month_id = ${MONTH_ID}
                     group by trmnl_brand, trmnl_model) c
            on a.trmnl_model = c.trmnl_model
  ...
  left join (select dm_model,
                    ...
                    core_num,
                    dm_net_type
               from termbstl.devinfo) e
    on h.trmnl_model = e.dm_model;"
