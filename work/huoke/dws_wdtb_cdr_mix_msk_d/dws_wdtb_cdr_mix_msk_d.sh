...
###############配置区############################################################################
#脚本名称
LOGNAME="dws_wdtb_cdr_mix_msk_d"
...
#正常执行hql
SQL="insert overwrite table dws_m.dws_wdtb_cdr_mix_msk_d partition (prov_id,month_id,day_id)
select mdn,
       other_party,
       call_type,
       biz_type,
       ...
       calling_home_code,
       called_home_code,
       calling_visit_area calling_visit_code,
       called_visit_code,
       third_home_code,
       third_visit_code,
       corporation_id,
     ...
     substr(day_id,1,6),
       day_id
  from dwi_m.dwi_evt_bill_oth_wash_cdr_msk_d
 where day_id = '${DAY_ID}'
   and prov_id = '${PROV_ID}'
union all
-- oidd   剔除cdr中存在交叉的数据
select t3.mdn,
       ...
       t3.end_time,
       t3.call_dur,
       t3.calling_home_code,
       t3.called_home_code,
       t3.calling_visit_code,
       t3.called_visit_code,
       t3.third_home_code,
       t3.third_visit_code,
       t3.corporation_id,
       t3.source_type,
       t3.prov_id,
     substr(t3.day_id,1,6),
       t3.day_id
  from (select mdn,
               other_party,
               call_type,
               ...
               prov_id,
               day_id
          from dwi_m.dwi_evt_bill_oth_oidd_cdr_msk_d
         where day_id = '${DAY_ID}'
           and prov_id = '${PROV_ID}') t3
  left join (select t1.mdn,
                    t1.other_party,
                    t1.call_type,
                    t1.biz_type,
                    t1.roam_type,
               ...
               from (select mdn,
                    ...
                    (select mdn,
                       ...
                       from dwi_m.dwi_evt_bill_oth_wash_cdr_msk_d
                      where day_id = '${DAY_ID}'
                        and prov_id = '${PROV_ID}') t2
              where t1.mdn = t2.mdn
                and t1.other_party = t2.other_party
                and t1.start_time < t2.end_time
                and t1.end_time > t2.start_time) t4
    on t3.mdn = t4.mdn
   and t3.other_party = t4.other_party
   and t3.start_time = t4.start_time
 where t4.mdn is null
union all
-- 剔除cdr 和   oidd  中存在交叉的数据
select t3.mdn,
       t3.other_party,
       t3.call_type,
       t3.biz_type,
  ...
  from (select mdn,
               ...
               prov_id,
               day_id
          from dwi_m.dwi_evt_bill_oth_wcdr_cdr_msk_d
         where day_id = '${DAY_ID}'
           and prov_id = '${PROV_ID}') t3
  left join (select t1.mdn,
                    t1.other_party,
