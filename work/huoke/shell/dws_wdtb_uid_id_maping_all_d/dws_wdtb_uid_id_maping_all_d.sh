...
###############配置区############################################################################
#脚本名称
LOGNAME="dws_wdtb_uid_id_maping_all_d"
...
function CONFIGURE(){
    DAY_ID=$1
    MONTH_ID=${DAY_ID:0:6}
	PRE_DAY_ID=$(date -d "${DAY_ID} -1 day" +%Y%m)
...
#正常执行hql
SQL="insert overwrite table dws_integ.dws_wdtb_uid_id_maping_all_d partition (prov_id, day_id)
select if(t2.mdn_md5 is null, t1.mdn, t2.mdn),
       if(t2.mdn_md5 is null, t1.mdn_md5, t2.mdn_md5),
       if(t2.mdn_md5 is null, t1.id, t2.id),
       if(t2.mdn_md5 is null, t1.id_md5, t2.id_md5),
       if(t2.mdn_md5 is null, t1.id_type, t2.id_type),
       '${PROV_ID}',
       '$DAY_ID'
  from (select mdn, mdn_md5, id, id_md5, id_type, prov_id, day_id
          from dws_integ.dws_wdtb_uid_id_maping_all_d
         where prov_id = '${PROV_ID}'
           and day_id = '${PRE_DAY_ID}') t1
  full join (select temp1.mdn,
                    temp1.mdn_md5,
                    temp1.id,
                    temp1.id_md5,
                    temp1.id_type,
               ...
               from (select mdn,
                            ...
                            prov_id,
                            day_id,
                            row_number() over(partition by mdn_md5, id_type order by id_source desc) rn
                       from dws_m.dws_wdtb_uid_id_maping_update_msk_d
                      where prov_id = '${PROV_ID}'
