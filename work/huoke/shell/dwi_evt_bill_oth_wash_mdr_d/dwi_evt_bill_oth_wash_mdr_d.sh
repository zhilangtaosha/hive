...
###############配置区############################################################################
#脚本名称
LOGNAME="dwi_evt_bill_oth_wash_mdr_d"
...
#正常执行hql
SQL="insert overwrite table dwi_integ.dwi_evt_bill_oth_wash_mdr_d partition (prov_id,day_id)
select t1.msisdn,
       t1.other_party,
       t1.call_type,
       t1.biz_type,
  ...
  from (select msisdn,
               other_party,
              (case  when call_type='01' then 'MO'
               when call_type='02' then 'MT' end) call_type,
               biz_type,
               imsi,
               timestamp,
               prov_id,
               day_id
          from cdrmblbstl.intf_oth_mdr
         where prov_id = '${PROV_ID}'
           and day_id = '${DAY_ID}') t1
 group by t1.prov_id,
          t1.day_id,
          t1.msisdn,
          t1.other_party,
