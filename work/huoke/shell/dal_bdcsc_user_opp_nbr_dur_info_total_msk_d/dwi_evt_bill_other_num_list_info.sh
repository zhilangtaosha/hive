...
###############配置区############################################################################
#脚本名称
LOGNAME="dwi_evt_bill_other_num_list_info"
...
#################################FOR循环开始##########################################################
###############################以下为SQL编辑区########################################################
NBR_SQL="create table IF NOT EXISTS dwi_integ.dwi_evt_bill_other_num_list_info_temp(
nbr string
)ROW FORMAT SERDE 
...
LOCATION
'/daas/motl/dwi/integ/evt/bill/dwi_evt_bill_other_num_list_info_temp';
load data local inpath '${baseDirForScriptSelf}/temp.txt' overwrite into table dwi_evt_bill_other_num_list_info_temp;
"
RunScript "${NBR_SQL}"
...
insert overwrite table dwi_integ.dwi_evt_bill_other_num_list_info
select concat(t1.mobile_num,lpad(t2.nbr,4,'0')) mdn,
md5(concat(t1.mobile_num,lpad(t2.nbr,4,'0'))) mdn_md5
from (select mobile_num from dwi_integ.dwi_evt_bill_mbl_num_att_d where corporation_name not like '%电信%') t1 inner join dwi_integ.dwi_evt_bill_other_num_list_info_temp t2;"

RunScript "${SQL}"

#删除临时表
DEL_SQL ="hive -e "drop table  dwi_integ.dwi_evt_bill_other_num_list_info_temp;""
RunScript "${DEL_SQL}"

done
Mergefile "${USERNAME}" "dwi_evt_bill_other_num_list_info" ""
