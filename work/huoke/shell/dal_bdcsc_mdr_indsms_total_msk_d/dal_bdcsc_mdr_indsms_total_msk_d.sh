...
###############配置区############################################################################
#脚本名称
LOGNAME="dal_bdcsc_mdr_indsms_total_msk_d"
...
#正常执行hql
SQL="insert overwrite table dal_bdcsc.dal_bdcsc_mdr_indsms_total_msk_d partition (day_id)
select t1.mdn, t1.calling_cnt, t1.called_cnt, t1.day_id
  from (select t.mdn, t.calling_cnt, t.called_cnt, day_id
  from (select mdn,
               count(distinct(case when calling_sms_cnt > 0 then other_party end)) calling_cnt,
               count(distinct(case when called_sms_cnt > 0 then other_party end)) called_cnt,
               day_id
          from dal_bdcsc.dal_bdcsc_mdr_indsms_msk_d
         where day_id = '${DAY_ID}'
         group by mdn,day_id
        union all
        select other_party mdn,
               count(distinct(case when called_sms_cnt > 0 then mdn end)) calling_cnt,
               count(distinct(case when calling_sms_cnt > 0 then mdn end)) called_cnt,
               day_id
          from dal_bdcsc.dal_bdcsc_mdr_indsms_msk_d
         where day_id = '${DAY_ID}'
         group by other_party,day_id) t
 group by t.mdn, t.calling_cnt, t.called_cnt, day_id) t1
  left join dwi_integ.dwi_evt_bill_other_num_list_info t2
    on t1.mdn = t2.mdn_md5
	where t2.mdn_md5 is null;"
