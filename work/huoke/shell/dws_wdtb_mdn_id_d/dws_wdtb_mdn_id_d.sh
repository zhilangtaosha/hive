...
#正常执行hql
SQL="insert overwrite table dws_integ.dws_wdtb_mdn_id_d partition (day_id)
select mdn,
       id_md5,
       (case
         when id_type = 'MDN' then
          '01'
         when id_type = 'IMSI' then
          '02'
         when id_type = 'MEID' or id_type = 'IMEI' then
          '03'
         when id_type = 'IDFA' then
          '04'
       end) type,
       day_id
  from dws_integ.dws_wdtb_uid_id_maping_all_d
 where day_id = '${DAY_ID}'
   and id_type in ('MDN', 'IMSI', 'MEID', 'IMEI', 'IDFA');"
