#!/bin/bash
#***********************************************************************************
# **  文件名称: dws_integ.dws_sev_user_creditrent_monitor_consume_m.sh
# **  创建日期: 2018年4月12日
# **  编写人员: yangyong
# **  输入信息: 
# **  输出信息: 
...
###############配置区############################################################################
#脚本名称
LOGNAME="dws_integ.dws_sev_user_creditrent_monitor_consume_m"
...
SQL="
INSERT OVERWRITE TABLE dws_integ.dws_sev_user_creditrent_monitor_consume_m PARTITION(prov_id,MONTH_ID)
SELECT CM.mdn,
       AC.owe_fee,
       AC.balance,
       AC.writeoff_fee,
  ...
  LEFT JOIN (SELECT *
               FROM dwi_integ.dwi_act_acct_user_info_m
              WHERE month_id = ${PRE_MONTH_ID}) AC
    ON CM.mdn = AC.accs_nbr
  LEFT JOIN (SELECT mdn, count(distinct other_party) as opc
               FROM dws_integ.dws_dtfs_contact_circle_score_m
              WHERE month_id = ${MONTH_ID}
                AND intimacy_score > 0
              GROUP BY mdn) CC
    ON CM.mdn = CC.mdn
  LEFT JOIN (SELECT *
               FROM dwi_integ.dwi_evt_bill_oth_ddr_m
              WHERE month_id = ${MONTH_ID}) DDR
    ON CM.mdn = DDR.mdn;"
