#!/bin/bash
#***********************************************************************************
# **  文件名称: dws_integ.dws_evt_bill_creditrent_monitor_cdr_m.sh
# **  创建日期: 2018年4月13日
# **  编写人员: yangyong
# **  输入信息: 
# **  输出信息: 
...
###############配置区############################################################################
#脚本名称
LOGNAME="dws_integ.dws_evt_bill_creditrent_monitor_cdr_m"
...
INSERT OVERWRITE TABLE dws_integ.dws_evt_bill_creditrent_monitor_cdr_m PARTITION(prov_id,MONTH_ID)
SELECT data.mdn,
       sum(case
             when data.call_dur is null then
              0
             else
              data.call_dur
           end), --通话时长
       count(case
               when length(data.other_party) > 0 then
                data.other_party
             end), --通话次数
       sum(IF(data.call_type = '01', 1, 0)), --主要号码个数
       sum(IF(data.call_type = '02', 1, 0)), --被叫号码个数
       sum(IF(data.call_type = '02' and data.call_dur > 30, 1, 0)), --有效被叫号码个数
       data.mdn_is_lack, --是否黑名单
       sum(IF(data.other_mdn_is_lack = 1, 1, 0)), --黑名单通话次数
       sum(IF(data.call_type = '01' and
              (data.other_party = '6412121CBB2DC2CB9E460CFEE7046BE2' or
              data.other_party = 'A17479231DC298309A3FDA7D7D00111A'),
              1,
              0)), --主叫10086或者10010次数
       sum(IF(data.call_type = '02' and
              (data.other_party = '6412121CBB2DC2CB9E460CFEE7046BE2' or
              data.other_party = 'A17479231DC298309A3FDA7D7D00111A'),
              1,
              0)), --被叫10086或者10010次数
       count(distinct(case
                        when data.roam_type <> '0' then
                         data.visit_area_code
                      end)), --漫游城市数
       count(distinct(case
                        when data.roam_type <> '0' then
                         substr(data.start_time, 1, 8)
                      end)), --漫游城市天数
       data.prov_id,
       data.month_id
  FROM (SELECT merged.mdn,
               merged.call_type,
               merged.roam_type,
               merged.other_party,
               merged.call_dur,
               IF(TB.mdn IS NOT NULL, 1, 0) mdn_is_lack,
               IF(black.mdn IS NOT NULL, 1, 0) other_mdn_is_lack,
               merged.visit_area_code,
               merged.start_time,
               merged.prov_id,
               merged.month_id
          FROM (SELECT CM.mdn,
                  ...
                  LEFT JOIN (SELECT *
                    ...
                    ON md5(CM.mdn) = adr.mdn) merged
          LEFT JOIN (SELECT *
                      FROM dws_i_mid.dws_dtfs_othnet_telecom_black_m_mid
                     WHERE month_id = ${PRE_MONTH_ID}) TB
            ON merged.mdn = TB.mdn
          LEFT JOIN (SELECT DISTINCT AB.mdn
                      FROM (SELECT mdn
                              FROM dws_i_mid.dws_dtfs_othnet_telecom_black_m_mid
                             WHERE month_id = ${PRE_MONTH_ID}
                            UNION ALL
                            SELECT mdn
                              FROM dws_i_mid.dws_dtfs_othnet_black_self_m_mid
                             WHERE month_id = ${PRE_MONTH_ID}) AB) black
            ON merged.mdn = black.mdn) data
 GROUP BY data.mdn, data.mdn_is_lack, data.prov_id, data.month_id;"
