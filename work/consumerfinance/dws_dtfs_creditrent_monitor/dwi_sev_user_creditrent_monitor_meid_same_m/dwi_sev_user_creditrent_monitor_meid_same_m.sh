#!/bin/bash
#***********************************************************************************
# **  文件名称: dwi_integ.dwi_sev_user_creditrent_monitor_meid_same_m
# **  创建日期: 2018年4月12日
# **  编写人员: yangyong
# **  输入信息: 
# **  输出信息: 
...
###############配置区############################################################################
#脚本名称
LOGNAME="dwi_integ.dwi_sev_user_creditrent_monitor_meid_same_m"
...
SQL="
INSERT OVERWRITE TABLE dwi_integ.dwi_sev_user_creditrent_monitor_meid_same_m PARTITION(prov_id,MONTH_ID)
SELECT CM.mdn,
       count(case
               when substr(rawdt.rgst_dt, 1, 6) = '${PRE_MONTH_ID}' then
                1
             end), -- 插拔卡次数
       count(case
               when substr(rawdt.rgst_dt, 1, 6) = '${PRE_MONTH_ID}' and
                    rawdt.meid = CM.meid then
                1
             end), -- t-2机卡匹配次数
       if(sum(case
                when rawdt.rgst_dt > cm.start_date and rawdt.meid = CM.meid then
                 1
                else
                 0
              end) > 0,
  ...
  LEFT JOIN (SELECT accs_nbr,
                    meid,
                    from_unixtime(unix_timestamp(rgst_dt,
                                                 'yyyy-MM-dd HH:mm:ss'),
                                  'yyyyMMddHHmmss') rgst_dt
               FROM termbstl.rawdt temp1
              where temp1.provinceid in
                    (select temp2.rawdt_id
                       from dim.dim_prov_match temp2
                      where temp2.prov_id = '${PROV_ID}')) rawdt
    ON CM.mdn = rawdt.accs_nbr
