...
###############配置区############################################################################
#脚本名称
LOGNAME="dwi_evt_bill_oth_wcdr_cdr_msk_d"
...
#正常执行hql
SQL="insert overwrite table dwi_m.dwi_evt_bill_oth_wcdr_cdr_msk_d partition (prov_id,day_id)
select t1.mdn, --计费号码
       t1.other_party, --对端号码
       t1.start_time,
       t1.end_time,
       t1.during call_dur, --通话时长
       t1.city_id calling_visit_code, --计费号码到访地
       '' called_visit_code, --对端号码到访地
       t1.imsi,
       t1.provinceid prov_id,
       t1.day
  from (select mdn_mi      mdn, --计费号码
               ...
               provinceid,
               day
          from wcdrbstl.wcdr_v2
         where type = '1'
           and provinceid = '${PROV_ID}'
           and day = '${DAY_ID}') t1
